...
struct rdft_plan_t {
  // degree of freedom
  size_t nitems;
  // perform the same FFTs for several times
  size_t repeat_for;
...
#pragma omp parallel for
  for (size_t j = 0; j < repeat_for; j++) {
    double * const xs_j = xs + j * nitems;
    double * const zs_j = zs + j * nitems;
    // compute complex dft to find Z_k
    dft(nh, 1, trigs, xs_j, zs_j);
    // compute FFT of the original real signal, corner cases
    {
      const double xe = zs_j[0];
      const double xo = zs_j[1];
      xs_j[ 0] = xe + xo;
      xs_j[nh] = xe - xo;
    }
    ...
    for (size_t k = 1; k < nh; k++) {
      ...
      const double complex z0 =
        + 0.5 * zs_j[2 * k + 0]
        + 0.5 * zs_j[2 * k + 1] * I;
      // compute 1/2 Z_{N / 2 - k}
      const double complex z1 =
        + 0.5 * zs_j[2 * (nh - k) + 0]
        + 0.5 * zs_j[2 * (nh - k) + 1] * I;
      // compute X_k^e = + 1/2 Z_k + 1/2 Z_{N / 2 - k}^*
      const double complex xe = +     z0 +     conj(z1);
      // compute X_k^o = - I/2 Z_k + I/2 Z_{N / 2 - k}^*
...
#pragma omp parallel for
  for (size_t j = 0; j < repeat_for; j++) {
    double * const xs_j = xs + j * nitems;
    double * const zs_j = zs + j * nitems;
    // create a complex signal, edge cases
    {
      const double x0 = 0.5 * xs_j[ 0];
      const double x1 = 0.5 * xs_j[nh];
