...
static int decide_dt_sur(
    const domain_t * const domain,
    double * const dt
) {
  const double dx = domain->dx;
  const double dy = domain->dy;
  *dt = sqrt(We / 2. / PI * pow(fmin(dx, dy), 3.));
  *dt *= safety_factors.sur;
  return 0;
}
...
int decide_dt(
  ...
  if (0 != decide_dt_sur(domain, &dt_sur)) {
    LOGGER_FAILURE("failed to find interfacial time-step constraint");
    goto abort;
  }
  *dt = fmin(fmin(dt_adv, dt_dif), dt_sur);
  return 0;
abort:
