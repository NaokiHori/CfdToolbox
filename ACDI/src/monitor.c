...
static int monitor_interface_field(
    const size_t step,
    const double time,
    const domain_t * const domain,
    const interface_field_t * const interface_field
) {
  const char file_name[] = ROOT_DIRECTORY "interface_field.dat";
  const size_t nx = domain->nx;
  const size_t ny = domain->ny;
  const double dx = domain->dx;
  const double dy = domain->dy;
  ...
  for (size_t j = 1; j <= ny; j++) {
    for (size_t i = 1; i <= nx; i++) {
      const double local_vof = vof[j][i];
      vof_min = fmin(vof_min, local_vof);
      vof_max = fmax(vof_max, local_vof);
      vof_sum += local_vof * dx * dy;
    }
  }
  return output(step, time, file_name, 3, (double []){vof_min, vof_max, vof_sum});
}
...
int monitor(
  ...
  if (0 != monitor_interface_field(step, time, domain, interface_field)) {
    LOGGER_FAILURE("failed to check / output interface field");
    goto abort;
  }
  return 0;
abort:
