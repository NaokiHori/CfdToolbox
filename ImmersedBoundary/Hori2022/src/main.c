...
int main(
  ...
  particle_stuffs_t particle_stuffs = {0};
  ...
  if (0 != particle_stuffs_init(&domain, &particle_stuffs)) {
    return 1;
  }
  ...
  for (double time = 0.; time < time_max; ) {
    ...
    double dt = 0.;
    if (0 != integrate(&domain, &flow_field, &flow_solver, &particle_stuffs, &dt)) {
      break;
    }
    step += 1;
    time += dt;
    if (next.monitor < time) {
      monitor(step, time, dt, &domain, &flow_field, &particle_stuffs);
      next.monitor += rate.monitor;
    }
    if (next.save < time) {
      static size_t id = 0;
      save(id, step, time, &domain, &flow_field, &particle_stuffs);
      id += 1;
      next.save += rate.save;
    }
  }
  ...
  if (0 != particle_stuffs_finalize(&particle_stuffs)) {
    return 1;
  }
  return 0;
}
