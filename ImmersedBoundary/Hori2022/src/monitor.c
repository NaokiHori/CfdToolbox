...
static int monitor_particle_stuffs(
    const size_t step,
    const double time,
    const particle_stuffs_t * const particle_stuffs
) {
  const char file_name[] = ROOT_DIRECTORY "particle_stuffs.dat";
  const particle_t * const particle = &particle_stuffs->particle;
  return output(
      step,
      time,
      file_name,
...
int monitor(
  ...
  if (0 != monitor_particle_stuffs(step, time, particle_stuffs)) {
